...
if __name__=='__main__':
    ...
    for ind in tqdm(df.index):
        text = df.loc[ind, 'Keywords'] + df.loc[ind, 'Name'] + df.loc[ind, 'RecipeInstructions']
        emb_dict[ind] = get_embedding(text, embedding_model)

    with open('./data/embedding_v2.json', 'w') as f:
